...
const httpTimeoutToSimulateDelay = 2000;
...
server.use(function(req, res, next) {
  setTimeout(next, httpTimeoutToSimulateDelay);
});
...
server.post("/plates/", function(req, res, next) {
  const error = validateNote(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.title); // Generate a slug for new plates.
    next();
  }
});
...
function validateNote(plate) {
  if (!plate.title) return "Title is required.";
  if (!plate.categoryId) return "Category is required.";
  if (!plate.content) return "Content is required.";
  return "";
}
